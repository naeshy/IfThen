...
IfThen_HiddenTooltip	= HiddenTooltip; --Global declaration
...
local CONST_ITEM_LEVEL2		= _G["ITEM_LEVEL_ALT"]; --Format "Item Level %d (%d)" This pattern works: "Item Level %d+ %((%d+)".
...
function HiddenTooltip:GetEquipmentItemInfo(itemLink, itemInfo)
	if (itemLink == nil or itemLink == "") then return nil; end --Itemlinks are in the format "Unit:Creature-num-num-num etc".
	itemInfo = strtrim(strupper(itemInfo)); --Can be: TITLE, ITEM_LEVEL, ITEM_LEVEL_PVP, ENCHANT, LEVEL_REQUIREMENT
	local strKey = strupper(itemLink);		--Removed: UPGRADE_LEVEL, UPGRADE_LEVEL_CURRENT, UPGRADE_LEVEL_MAX,
	...
	if (cache_ItemList ~= nil and cache_ItemList[strKey] ~= nil) then
		...
		return tbl[itemInfo]; --if the key specified does not exist then the item might not have that value, either way we return it (or nil in those cases)
	end
	...
	--Find the line that matches the "Item Level %d (%d)" string
	--tblLines[#tblLines] = "Item Level 333 (999 PvP)";
	p = self:FindTooltipLine_ITEM_LEVEL_PVP(tblLines);
	if (p~=nil) then tbl["ITEM_LEVEL_PVP"] = p; end

	--[[ Item upgrades removed in WOD
...
function HiddenTooltip:FindTooltipLine_ITEM_LEVEL_PVP(tblLines)
	--Get the PVP item level from the tooltip.
	local gsub = gsub;	--local fpointer
	local p = CONST_ITEM_LEVEL2;	--"Item Level %d (%d)";
	p = gsub(p, "%(", ""); 			--Remove the ( ) in the string since they are used in patterns
	p = gsub(p, "%)", "");
	p = gsub(p, "%%d", "(%%d+)");	--Replace %d with (%d+)

	for i=1, #tblLines do
		local s = tblLines[i];		--"Item Level 333 (999 PvP)";
		s = gsub(s, "%(", "");		--Remove the ( ) since they are used in patterns
		s = gsub(s, "%)", "");

		local a,b = strmatch(s, p); --Second match (b) should be PVP item level
		if (a ~= nil and b ~= nil) then return b; end
	end--for
	return nil; --Reach here and the pattern wasn't found
end
